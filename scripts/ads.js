...
(function() {
	...
	.close_btn {\
	    background: url("http://www.sinaimg.cn/hs/sinahouse/images/adb.png") no-repeat scroll -25px -110px transparent;\
	    height: 20px;\
	    width: 20px;\
	}\
	...
	this.ads = {
		...
		onNewVideoClosed:function(id){
			...
			ads.newvideocompnum =0;
			ads.newvideoplaystatus['video'+id.split('_')[1]] = 'pause';
			var player = document.getElementById(id.split('_')[0]);
			ads.animate(player, {
						width: "22px"
					}, {
						duration : 500,
			...
			setTimeout(function(){
				ads.newvideodisplay = false;
			},500);
            
		},
		onNewVideoComplete:function(id){
			
			//记录每个视频是否播放完毕,因为只有1个关闭按钮,点击关闭按钮另一个可能还在播放就隐藏了
			//如果展开的时候有1个是正在播放的,那么不需要再次执行playagin
			ads.newvideoplaystatus['video'+id.split('_')[1]] = 'complete';
			ads.newvideocompnum++;
			if(!ads.newvideodisplay){
				ads.newvideocompnum = 0;
			}
			//当视频数量为2个时 才会执行全部播放完才回缩的逻辑
			if(ads.newvideolength > 1){
				if(ads.newvideocompnum>1 ){
					ads.newvideocompnum = 0;
					var player = document.getElementById(id.split('_')[0]);
					ads.animate(player, {
								width: "22px"
		...
		onTearOpen:function(str){
			var id = str.split("|")[0];
			var zindex = str.split("|")[1];
			var tear = document.getElementById(id);
			ads.setStyle(tear,{zIndex:zindex});
		},
		onTearClose:function(str){
			var id = str.split("|")[0];
			var tear = document.getElementById(id);
			ads.setStyle(tear,{zIndex:0});
		}
	};
	ads.cssProps = {
